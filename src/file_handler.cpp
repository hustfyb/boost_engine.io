...
void file_handler::operator()(const Request& req, Response& res,function<void(system::error_code, std::size_t)> cb)
{
  // Decode url to path.
  std::string request_path;
  if (!url_decode(req.url, request_path))
  {
    res.stock_reply(Response::bad_request);
    return;
  }
  ...
  if (!is)
  {
    res.stock_reply(Response::not_found);
    return;
  }
